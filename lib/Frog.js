...
class Frog extends Square {
  ...
  collisionDetection(canvas, logArray, turtleArray, carArrayLeft, lillypadArray, truckArray, resetGame, newScore) {
    ...
    if (this.x < 0 || this.x + this.width > canvas.width || this.y < 0) {
      ...
      this.lillyCollide(lillypadArray,resetGame, newScore);
 ...
 this.y > 0 && this.collide === false && this.turtleCollide === false && this.winner === false) {
  ...
  lillyCollide(array, resetGame, newScore) {
    let frog = this
