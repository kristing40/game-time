...
function generateLillypads() {
  ...
  for (var i = 0; i < 5; i++) {
    var lillypads = new Lillypad(x, 50, 80, 30);
...
function generateTurtles() {
  ...
  for (var i = 0; i < 6; i++) {
    var turtles = new Turtle(x, 80, 80, 30, 4);
...
function generateLogs() {
  ...
  for (var i = 0; i < 6; i++) {
    var logs = new Log(x, 170, 85, 30, 3);
...
function generateRightTrucks() {
  ...
  for (var i = 0; i < 4; i++) {
    var trucks = new Truck(x, 430, 100, 40, 3);
...
function generateLeftCars() {
  ...
  for (var i = 0; i < 4; i++) {
    var cars = new Car(x, 300, 100, 40, 3);
...
requestAnimationFrame(function gameloop() {
  ...
  if (currentGameState === 'game-on') {
    ...
    context.drawImage(logo, 330, 535, 250, 70);
    context.fillText('Lives:', 745, 570);
    context.fillText(playerFrog.lives, 850, 570);

    playerFrog.collisionDetection(canvas, turtleArray, logArray, carArrayLeft, truckArray, resetGame);
  }
