...
const playerFrog = new Frog(430, 380, 40, 40);
...
const generatingLogs = generateLogs();
const generatingTurtles = generateTurtles();
const generatingLeftCars = generateLeftCars();
const generatingRightTrucks = generateRightTrucks();
...
window.addEventListener('click', function() {
  if (currentGameState === 'ready') {
    ...
    context.fillText('PAUSE', 300, 300);
...
requestAnimationFrame(function gameloop() {
  ...
  if (currentGameState === 'game-on') {
    ...
    // if (playerFrog.collisionDetection()) {
    //   resetGame()
    // }
    winner();
  }
